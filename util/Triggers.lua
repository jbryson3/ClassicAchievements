...
local function updateItemsInInventory()
    ...
    for id, quantity in pairs(items) do
        ...
        elseif id == 210795 or id == 211502 or id == 211423 or id == 210794 then
            trigger(TYPE.SOD_P1_PROF_EPIC, nil, 1, true)
        end
    end
end
...
local function checkForThanks()
    ...
    local thanks = {
        -- me
        ['Махич-Пламегор'] = true,
        ['Darktank-LivingFlame'] = true,
        ['Brownessa-LivingFlame'] = true,
...
local events = {
    ...
    UPDATE_BATTLEFIELD_SCORE = function()
        ...
        if IsSOD() and lvl == 25 or lvl == 60 then
