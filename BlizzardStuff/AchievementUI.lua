...
-- 3 for also stats
-- Do not forget to also enable additional tabs in XML
local tabs = 1
...
ACHIEVEMENTUI_SUMMARYCATEGORIES = {1, 2, 5, 8, 9, 10};
...
local CRITERIA_TYPE_COMPLETE_ACHIEVEMENT = 20
...
function AchievementFrame_SetTabs()
	if tabs >= 2 then AchievementFrameTab2:Show() end
	if tabs == 3 then AchievementFrameTab3:SetPoint("LEFT", AchievementFrameTab2, "RIGHT", -5, 0) end
end
...
local function RegisterCustomEvent(eventName)
	customEvents[eventName] = true
end

local function UnregisterCustomEvent(eventName)
	customEvents[eventName] = nil
end

function AchievementFrameCategories_OnCustomEvent(self, event, ...)
	if customEvents[event] then AchievementFrameCategories_OnEvent(event, ...) end
end
...
function AchievementFrameAchievements_OnEvent (self, event, ...)
	...
	if ( event == "ADDON_LOADED" ) then
		...
		RegisterCustomEvent("CRITERIA_UPDATE");
...
function AchievementObjectives_DisplayCriteria (objectivesFrame, id, renderOffScreen)
	...
	for i = 1, numCriteria do
		...
		if ( criteriaType == CRITERIA_TYPE_COMPLETE_ACHIEVEMENT and assetID ) then
			metas = metas + 1;
...
function AchievementFrameSummaryCategory_OnHide (self)
	UnregisterCustomEvent("ACHIEVEMENT_EARNED");
end
...
function AchievementFrameComparison_OnLoad (self)
	...
	RegisterCustomEvent("ACHIEVEMENT_EARNED");
	RegisterCustomEvent("INSPECT_ACHIEVEMENT_READY");
...
function AchievementFrameComparison_Update ()
	...
	for i = 1, numButtons do
		...
		if i > numAchievements then buttons[i]:Hide() else buttons[i]:Show() end
	end
