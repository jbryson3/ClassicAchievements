    ...
    return {
        ...
        CompleteCriteria = function(self, achievementID, criteriaID, withQuantity)
            ...
            if achievement ~= nil then
                local criteria
                local total = 0
                for _, c in pairs(achievement:GetCriterias()) do
                    if c.id == criteriaID then criteria = c end
                    total = total + 1
                end
                if total > 0 and criteria ~= nil then
                    AchievementFrameAchievements_Update()
                    CriteriaAlertSystem:AddAlert(achievementID, criteria.name)
                    PlaySound(SOUNDKIT.UI_IG_STORE_PURCHASE_DELIVERED_TOAST_01)
                end
            end
            return true
